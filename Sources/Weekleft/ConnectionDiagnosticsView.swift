...
@MainActor final class ConnectionDiagnostics: ObservableObject {
    ...
    func check(store: AppStore, sessions: SessionStore, provider requestedProvider: ProviderID? = nil) async {
        guard !busy else { return }; busy = true; reportText = nil; copied = false
        defer { busy = false }
        while store.refreshing && !Task.isCancelled { try? await Task.sleep(for: .milliseconds(100)) }
        guard !Task.isCancelled else { return }
        await store.refresh(provider: requestedProvider); await sessions.refresh(); sessions.updateHookConfiguration()
        let providers = store.providers.filter { requestedProvider == nil || $0 == requestedProvider }
        let claude = SessionSources.discoverClaude(), codex = FileManager.default.isExecutableFile(atPath: store.codexPath) ? store.codexPath : nil
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                            ...
                            if let repair = result.repair {
                                Button(L(repair.title)) {
                                    if repair == .refresh || repair == .checkSignIn {
                                        Task { await diagnostics.check(store: store, sessions: sessions, provider: result.provider) }
                    ...
                    if !diagnostics.results.isEmpty {
                        Divider()
                        Text(L("Отчёт содержит только версии Lunavect и macOS, состояние подключений и коды ошибок. Без переписки, названий сессий, путей и токенов. Ничего не отправляется автоматически."))
            ...
            HStack {
                ...
                if let date = diagnostics.checkedAt { Text(L("Проверено {0}", date.formatted(date: .omitted, time: .shortened))).font(.system(size: 11)).foregroundStyle(.secondary) }
                Spacer()
                if diagnostics.busy { ProgressView().controlSize(.small) }
                Button(L(diagnostics.results.isEmpty ? "Проверить подключение" : "Проверить снова")) { Task { await diagnostics.check(store: store, sessions: sessions) } }
