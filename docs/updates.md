...
```sh
./scripts/distribute.sh archive 0.1.7 179 /path/to/published-appcast.xml
./scripts/distribute.sh submit 0.1.7 179
# After Apple's notarization completes:
./scripts/distribute.sh export 0.1.7 179
```
...
/path/outside-repository/dmg-venv/bin/python scripts/package-dmg.py \
  --app '/path/to/Notarized-179/Lunavect.app' \
  --output '/path/to/release-assets/Lunavect-0.1.7.dmg'
```
