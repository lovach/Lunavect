...
```sh
./scripts/distribute.sh archive 0.1.5 162 /path/to/published-appcast.xml
./scripts/distribute.sh submit 0.1.5 162
# After Apple's notarization completes:
./scripts/distribute.sh export 0.1.5 162
```
...
/path/outside-repository/dmg-venv/bin/python scripts/package-dmg.py \
  --app '/path/to/Notarized-162/Lunavect.app' \
  --output '/path/to/release-assets/Lunavect-0.1.5.dmg'
```
