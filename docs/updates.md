...
The six `WidgetRegistrationTests` cover version/path changes, transient and persistent failures, cancellation, excluded bundles and signalling only a synthetic extension at the exact matching executable path. An actual Sparkle download/install cycle and future macOS releases still require separate verification. The integration uses Apple's [LSRegisterURL](https://developer.apple.com/documentation/coreservices/1446350-lsregisterurl) and Sparkle's [pre-install delegate callback](https://sparkle-project.org/documentation/api-reference/Protocols/SPUUpdaterDelegate.html).
