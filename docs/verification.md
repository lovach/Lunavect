...
| Installed build | The app from the release DMG was installed over local build 185 with `install.sh`. Hook events kept arriving after launch and the `SubagentStop` handler stayed in Claude Code's settings. Two medium desktop widgets, limits and activity, rendered LIVE after installation. At the app's 10-minute registration confirmation macOS dropped the widget extension ("LS doesn't have a containing bundle") while notarized export copies of Lunavect were still registered with Launch Services, and the widgets showed placeholders. After those copies were unregistered and the installed widget was registered again, both widgets were LIVE and stayed LIVE through a full relaunch schedule, including the 10-minute confirmation. |
| Remaining scope | A complete Sparkle installation cycle and other macOS versions remain unverified. Inside a running turn the background count can include commands that already finished: Claude Code reports exact background work only when a turn ends. |
