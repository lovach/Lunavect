...
| `python3 scripts/check-native-renders.py --run --require-render --suite legacy-values --output …` | Same isolation probes, preview composition/stop, four migrated legacy value-view methods and exact 12-image RU/DE matrix. | Whole Settings/ReleaseScreenshots or native window controls; these remain guarded and excluded. |
...
The initial 16-image matrix covers RU and DE, light and dark, current limits, unavailable limits, stale Claude with expired five-hour allowance next to fresh Codex, and the application's selected activity chart with a gap and a partial current hour. The fixture uses fixed dates and in-memory values. It deliberately does not exercise a whole SettingsView or sessions store.
...
Signing configuration, installed apps, Codex configuration, accounts and the selected Tide artwork are outside this automation change. Broader integration and publication remain separate work with their own evidence.
