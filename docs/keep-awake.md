...
The automatic preference persists across launches. It never grants macOS permission by itself. After a failure, the panel shows the reason and waits for **Retry connection** rather than repeatedly re-enabling the mode. Turn off the main switch to disarm automatic mode as well.
...
An app update can invalidate macOS's reference to the previous helper executable. Before first use in a new build, Lunavect refreshes the service registration. If an already-approved helper cannot be reached, it retries registration once. Pending permission and invalid signatures are not bypassed.

By default, battery charge at 10% or below on battery power, or serious thermal pressure, stops the mode. **Settings → Keep Awake → Stop conditions** exposes independent battery and thermal switches, a battery threshold from 5% to 50%, and whether battery operation is allowed. The current policy is sent to the authenticated helper. Changes apply to an active mode immediately; a newly triggered stop is shown in the panel and does not cause an automatic restart loop. Disabling these optional conditions affects Lunavect only; it does not alter macOS thermal protections.
